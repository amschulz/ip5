...
df_read = pd.read_csv(filepath_or_buffer='outputs/SARIMA_Pipeline_ParameterSearch.csv',
                 ...
                 header=0,
                 index_col=0)
df_hortima = pd.read_csv(filepath_or_buffer='outputs/SARIMA_Pipeline_8_Jahre.csv',
                         sep=';',
                         header=0,
                         index_col=0)
df_read = df_read.drop(columns=['TrainData Mean'])
...
for ind, artId in enumerate(artIds):
    ...
    for i in range(1, 11):
        ...
        hortima_data.append(df_read['Grid M%s' % i][artId])
        if ind == 0:
            drop_col.append('Test M%s' % i)
            drop_col.append('Forecast M%s' % i)
            drop_col.append('Grid M%s' % i)
...
df_read['MA Mape Grid'] = ma_mape_hortima
...
plt.title("Vergleich Grid-Projektteam: Boxplot-Werte")
